...
const PROJECTS_DATA = [
	{
		...
		role: 'Project Lead',
		description:
			'An e-commerce platform designed for purchasing affordable electronics globally',
		points: [
	...
	},
	{
		...
		description:
			"An internal tool for Dr. Reddy's brand managers to identify high-value doctors and plan marketing campaigns",
		points: [
		...
		technologies: ['NestJS', 'Postgres', 'Redis', 'React', 'Docker', 'AWS'],
	},
	{
		...
		description:
			'A popular Indian content streaming platform used by millions',
		points: [
		...
		technologies: ['Node.js', 'Redis (Cache + DB)', 'AWS', 'OpenTelemetry'],
	},
	{
		title: 'ACadru',
		...
		role: 'Fullstack developer',
		description:
			'An Ed-Tech platform helping students prepare for their careers',
		points: [
	...
	},
	{
		...
		role: 'Backend developer',
		description:
			'A real estate app for brokers and owners to manage and share properties',
		points: [
							...
							{proj.points?.length > 0 && (
								<ol className='ml-6 list-decimal mb-2'>
									{proj.points?.map((point, pointIndex) => (
										<li
											key={`${proj.title}-point-${pointIndex}`}
										>
											{point}
										</li>
									))}
